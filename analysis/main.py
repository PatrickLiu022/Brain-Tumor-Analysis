...
def main():
    ...
    while run:
        ...
        print()
        print('If you ran this section before on your local device, '
              'you can skip this part.')
        load_data_input = input(
          'Continue? Press any key to continue, n to stop: ')
        ...
        print()
        print('Plotting visualizations and training models ...')
        ...
        if user_input != 'n':
            ...
            bt_train_x, bt_test_x, bt_train_y, bt_test_y = train.get_train(
              bt_features, bt_labels)

            # evaluate accuracy
            plots.plot_accuracies(
              train.find_max_depth(
                bt_train_x, bt_train_y, bt_test_x, bt_test_y), 'max depth')
            plots.plot_accuracies(
              train.find_max_features(
                bt_train_x, bt_train_y, bt_test_x, bt_test_y), 'max features')
            plots.plot_accuracies(
              train.find_max_leaf_nodes(
                bt_train_x, bt_train_y, bt_test_x, bt_test_y
                ), 'max leaf nodes')
            acc_model = train.train_model(
              bt_train_x, bt_train_y, bt_test_x, bt_test_y)

            plots.plot_confusion(acc_model, bt_test_x, bt_test_y)
            train.plot_feature_importance(acc_model, bt_features)
            plots.plot_mean_feature(
              train.get_df(), 'original_glcm_ClusterShade')
        ...
        if user_input != 'n':
            ...
            index = input('Pick any of the 3064 images for the model to'
                          'predict on by typing a number from 1 - 3064: ')
            index = int(index) - 1
            image_scan, img_features, img_labels, mlp = train.identify_tumor(
              image, labels, masks, index)
            print('Generating prediction of mask'
                  'in image and label of true mask')
            masks_int = masks[index]
            masks_int = masks_int + 1
            plots.plot_tumor_identification(
              image_scan, labels, masks_int, img_features, img_labels, mlp)
